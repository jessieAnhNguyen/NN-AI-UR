...
public class LogisticClassifierWithDisplay {
	...
	public static void main(String[] argv) throws IOException {
		if (argv.length < 3) {
			System.out.println("usage: java LogisticClassifierWithDisplay data-filename nsteps alpha");
		...
		LogisticClassifier classifier = new LogisticClassifier(ninputs) {
			...
			public void trainingReport(List<Example> examples, int stepnum, int nsteps) {
				...
				display.addPoint(oneMinusError);
		...
		String newName = nameSplit[nameSplit.length-1].split("\\.")[0] + "-logistic-output.csv";
