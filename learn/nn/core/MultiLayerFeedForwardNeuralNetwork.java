...
abstract public class MultiLayerFeedForwardNeuralNetwork extends FeedForwardNeuralNetwork {
	...
	public void backprop(Example example, double alpha) {
		...
		int L = this.layers.length;
		Unit[] outputLayer = this.getOutputUnits();
		for (int j=0; j < outputLayer.length; j++){
			LogisticUnit outUnit = (LogisticUnit) outputLayer[j];
			double in_j = outUnit.getInputSum();
			this.layers[L-1][j].delta = outUnit.activationPrime(in_j) * (example.outputs[j] - outUnit.getOutput());
		}

		//Propagate deltas backwards from output layer to input layer
		for (int l = L-2; l > 0; l--){
			for (int i = 0; i < this.layers[l].length; i++){
				LogisticUnit hidUnit = (LogisticUnit) this.layers[l][i];
				double in_i = hidUnit.getInputSum();
				double sum = 0.0;

				for (Connection connection: hidUnit.outgoingConnections){
					sum += connection.weight * connection.dst.delta;
				}
				/*
				for (int j = 0; j < this.layers[l+1].length; j++){
					sum += this.layers[l][i].outgoingConnections.get(j).weight * this.layers[l+1][j].delta;
				}

				 */
				this.layers[l][i].delta = hidUnit.activationPrime(in_i) * sum;
			}
		}
		...
		for (int l=0; l < L-1; l++){
			for (int i=0; i < this.layers[l].length; i++){
				for (Connection connection: this.layers[l][i].outgoingConnections){
					connection.weight += alpha * connection.src.getOutput() * connection.dst.delta;
				}
			}
		}
		...
		for (int l=1; l<L; l++){
			for (int i=0; i< this.layers[l].length; i++){
				for (Connection connection: this.layers[l][i].incomingConnections){
					connection.weight += alpha * connection.src.getOutput() * connection.dst.delta;
				}
			}
		}
